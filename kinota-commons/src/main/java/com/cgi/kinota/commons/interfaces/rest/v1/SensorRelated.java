 ...
 */
@Component
@Path(Constants.API_VERSION_PATH + "/Sensors({id})/{navigationProperty}")
@Api(
        value = "API for CRUD of SensorThings Sensor objects.",
        consumes = MediaType.APPLICATION_JSON,
        produces = MediaType.APPLICATION_JSON
)
public class SensorRelated extends BaseResource {

    private static final Logger logger = LoggerFactory.getLogger(SensorRelated.class);
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Read related entities associated with a Sensor.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "The entity related to the Sensor bearing the specified ID was found."),
            @ApiResponse(code = 404, message = "An Sensor with this ID was not found."),
    ...
    public Response readRelated(
            @ApiParam(value = "The ID of the Sensor whose entity is to be read.", required = true) @PathParam("id") String id,
            ...
            switch (navigationProperty) {
                case Datastream.NAME_PLURAL:
                {
                    ...
                    return streamJsonResponse(Response.Status.OK,
                            g -> { QueryableService.toJsonArrayRelated(related.fetchRelatedDatastreamsForSensor(s.getId(), q), q, g, requestUriBase, entityUriBase,
    ...
    @Path("$ref")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Get a list of selfLInks for related entities associated with a Sensor.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "The entity related to the Sensor bearing the specified ID was found."),
        ...
        try {
            ...
            switch (navigationProperty) {
                case Datastream.NAME_PLURAL:
                {
                    String entityUriBase = this.getBaseUrlForEntity(uriInfo, Constants.API_VERSION, Datastream.NAME_PLURAL);
                    return streamJsonResponse(Response.Status.OK,
                            g -> { QueryableService.toJsonArrayRelatedRef(related.fetchDatastreamUuidsForSensor(s.getId(), q), q, g, requestUriBase, entityUriBase,
                                    Paginator.DEFAULT_PAGINATOR);});
                }
                default:
                    // Unknown related entity
