 ...
 */
@Component
@Path(Constants.API_VERSION_PATH + "/FeaturesOfInterest({id})/{navigationProperty}")
@Api(
        value = "API for CRUD of SensorThings FeatureOfInterest objects.",
        consumes = MediaType.APPLICATION_JSON,
        produces = MediaType.APPLICATION_JSON
)
public class FeatureOfInterestRelated extends BaseResource {

    private static final Logger logger = LoggerFactory.getLogger(FeatureOfInterestRelated.class);
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Read related entities associated with a FeatureOfInterest.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "The entity related to the FeatureOfInterest bearing the specified ID was found."),
    ...
    public Response readRelated(
            @ApiParam(value = "The ID of the FeatureOfInterest whose entity is to be read.", required = true) @PathParam("id") String id,
            @ApiParam(value = "The related entity to be read.", required = true) @PathParam("navigationProperty") String navigationProperty) {
        try {
            FeatureOfInterest foi = service.findOne(UUID.fromString(id));
            ...
            switch (navigationProperty) {
                case Observation.NAME_PLURAL:
                    ...
                    return streamJsonResponse(Response.Status.OK,
                            g -> { QueryableService.toJsonArrayRelated(related.fetchRelatedObservationsForFeatureOfInterest(foi.getId(), q), q,
    ...
    @Path("$ref")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Get a list of selfLinks for related entities associated with a FeatureOfInterest.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "The entity related to the FeatureOfInterest bearing the specified ID was found."),
            @ApiResponse(code = 404, message = "A FeatureOfInterest with this ID was not found."),
            @ApiResponse(code = 500, message = "The server encountered an internal error. Please retry the request.")})
        ...
        try {
            ...
            switch (navigationProperty) {
                case Observation.NAME_PLURAL:
                {
                    String entityUriBase = this.getBaseUrlForEntity(uriInfo, Constants.API_VERSION, Observation.NAME_PLURAL);
                    return streamJsonResponse(Response.Status.OK,
                            g -> { QueryableService.toJsonArrayRelatedRef(related.fetchRelatedObservationUuidsForFeatureOfInterest(foi.getId(), q), q, g, requestUriBase, entityUriBase,
