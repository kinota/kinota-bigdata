...
/**
 * Created by bmiles on 2/6/17.
 */
@Component
@Path(Constants.API_VERSION_PATH + "/HistoricalLocations")
@Api(
        value = "API for CRUD of SensorThings HistoricalLocation objects.",
        consumes = MediaType.APPLICATION_JSON,
        produces = MediaType.APPLICATION_JSON
)
public class HistoricalLocationCF extends BaseResource {

    private static final Logger logger = LoggerFactory.getLogger(HistoricalLocationCF.class);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Create an HistoricalLocation.")
    @ApiResponses(value = {
            @ApiResponse(code = 500, message = "The server encountered an internal error. Please retry the request.")})
    public Response createHistoricalLocation(
            @ApiParam(value = "JSON representation of the HistoricalLocation to be created.", required = true) JsonObject json) {
        try {
            HistoricalLocation hl = service.create(json);
            // Add selfLink to "Location" response header.
            return Response.created(hl.generateSelfLinkUrl(getBaseUrl())).build();
        } catch (Exception e) {
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Read HistoricalLocations.")
    @ApiResponses(value = {
            @ApiResponse(code = 500, message = "The server encountered an internal error. Please retry the request.")})
    public Response readHistoricalLocations() {
        try {
            ODataQuery q = parseODataQuery();
            String baseUrl = this.getBaseUrlForEntity(uriInfo, Constants.API_VERSION, HistoricalLocation.NAME_PLURAL);
