...
import static com.cgi.kinota.commons.domain.util.Serialization.stringToMimeType;
...
public class SensorServiceTest extends CassandraTestBase {
    ...
    public static Sensor createSensor(SensorService sensorSvc,
                                      String name,
                                      String description,
        ...
        Sensor s = null;
        try {
            MimeType encodingType = stringToMimeType(encodingTypeStr);
            URI metadata = stringToURI(metadataStr);
            s = sensorSvc.create(name, description, encodingType, metadata);
        } catch (Exception e) {
    ...
    public void testCreateSensor() throws Exception {
        Sensor s = createSensor(sensorSvc,
                "TMP36",
                "TMP36 - Analog Temperature sensor",
                "application/pdf",
                "http://example.org/TMP35_36_37.pdf");
        Sensor readSensor = sensorSvc.findOne(s.getId());
        assertEquals(s.getName(), readSensor.getName());
        assertEquals(s.getDescription(), readSensor.getDescription());
        assertEquals(s.getEncodingType().toString(), readSensor.getEncodingType().toString());
        assertEquals(s.getMetadata(), readSensor.getMetadata());
    }

    @Test
    public void createDeleteSensor() {
        ...
        // Delete Sensor
        related.deleteSensor(s.getId());
        // Make sure Sensor was deleted
        boolean exceptionThrown = false;
        try {
            sensorSvc.findOne(s.getId());
        ...
        // Make sure relationship between Sensor/Datastream and Datastream/Thing were removed
        ...
        Pair<Long, Iterable<UUID>> dsSen = related.fetchDatastreamUuidsForSensor(s.getId(),
                ODataQuery.defaultQuery());
        assertEquals(Long.valueOf(0l), dsSen.getLeft());
    }
